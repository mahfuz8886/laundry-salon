                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($income_lists as $income_list)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @foreach ($income_list->income_salon_transection as $item)
                                                            {{ $item->account_head->head_name .", " }}
                                                        @endforeach
                                                    </td>
                                                    <td>
                                                        <a class="pr-2" href="{{ route('superadmin.account.incomeSalonEdit', $income_list->id) }}"><i class="fa fa-edit text-primary"></i></a>
                                                        <a class="pr-2" href="{{ route('superadmin.account.incomeSalonDelete', $income_list->id) }}"><i class="fa fa-trash text-danger"></i></a>
                                                        <a class="pr-2" href="{{ route('superadmin.account.incomeSalonPDF', $income_list->id) }}"><i class="fa fa-file-pdf-o text-info"></i></a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
