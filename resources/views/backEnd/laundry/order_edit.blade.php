            ...
            @if ($order_items)
                <div class="package-edit">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row mb-4">
                                ...
                                <div class="col-md-3">
                                    <img class= "backend_image" src="{{ URL($order_items->package->image ?? 'public/avatar/avatar.png') }}" alt="">
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    ...
                                    <input disabled type="text" class="form-control" value="{{ $order_items->package->package_name ?? '' }}" name="package_name" id="" required>
                                </div>
                                <div class="col-md-3">
                                    <label for="supplier" class="form-label">Package Amount</label>
                                    <input disabled type="number" class="form-control" value="{{ $order_items->package->package_amount ?? '' }}" name="package_amount" id="" required>
                                </div>
                                <div class="col-md-3">
                                    <label for="supplier" class="form-label">Package Duration (day)</label>
                                    <input disabled type="number" class="form-control" value="{{ $order_items->package->duration ?? '' }}" name="package_duration" id="" required>
                                </div>
                                <div class="col-md-3">
                                    <label for="supplier" class="form-label"> Max Quantity </label>
                                    <input disabled type="number" class="form-control" value="{{ $order_items->package->package_quantity ?? '' }}" name="package_quantity" id="" required>
                                </div>
                            </div>
                    ...
                    <div class="card">
                        ...
                        {{-- package edit --}}
                        <form action="{{ route('superadmin.laundry.update-order-package') }}" method="post">
                            @csrf
                            <input type="hidden" name="rowId" value="{{ $orderId }}">
                            <div class="card-body">

                                @php
                                    $laundry_pkg_items = App\LaundryPkgOrderItem::where('order_id', $order_items->order_id)->with('package')->get();
                                @endphp
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped w-100">
                                        ...
                                        <tbody class="rowContainer-package">
                                            {{-- @if($order_items->package->items) --}}
                                            @if($laundry_pkg_items)
                                                @foreach($laundry_pkg_items as $pItem)
                                                    <tr>
                                ...
                                <input type="submit" value="Submit" id="package-update" class="btn btn-primary mt-5">
                            </div>
                        </form>
                        {{-- package edit --}}
    ...
    <script>
        ...
        function loadProductService(pid) {
            ...
            if(productId != null) {
                $.ajax({
                    type: 'POST',
                    url: '{{ route('superadmin.laundry.getProductServices_for_edit') }}',
                    data: {productId},
                    success: function(data) {
                        if(data != null) {
                            console.log(data);
